...
let getRandomFromArray = (arr = []) => BUILD_TYPE[Math.floor(Math.random() * arr.length)];

let getRandomInteger = (min = 0, max = 1) => {
...
let createPin = (advert) => {
  let template = document.querySelector(`#pin`).content.querySelector(`.map__pin`).cloneNode(true);
  let img = template.getElementsByTagName(`img`)[0];
  img.setAttribute(`src`, advert.author.avatar);
  img.setAttribute(`alt`, advert.offer.title);
  template.setAttribute(`style`, `left: ${advert.location.x}px; top: ${advert.location.y}px`);
